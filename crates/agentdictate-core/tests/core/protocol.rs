...
fn rejected_commands_return_a_correlated_error_instead_of_looking_successful() {
    ...
    assert_eq!(
        ...
        r#"{"protocol_version":2,"message":"command_rejected","request_id":19,"error":"microphone unavailable"}"#
    );
}
...
fn history_page_requests_are_bounded_and_typed_on_the_wire() {
    ...
    assert_eq!(
        wire,
        r#"{"protocol_version":2,"command":"get_history_page","request_id":12,"request":{"search":"database migration","page_size":20,"after":"opaque-page-2"}}"#
    );
    assert_eq!(
