...
/// Returns the shared logical area of two desktop rectangles.
///
/// X11 commonly reports one work area for the full virtual desktop. Clipping
/// that rectangle to GPUI's primary display keeps placement monitor-local even
/// when the primary monitor does not start at the virtual origin.
pub fn intersect_logical_rects(first: LogicalRect, second: LogicalRect) -> Option<LogicalRect> {
    ...
    Some(LogicalRect::new(
        i32::try_from(left).expect("intersection origin comes from an i32 origin"),
        i32::try_from(top).expect("intersection origin comes from an i32 origin"),
        u32::try_from(right - left).expect("intersection cannot exceed either u32 width"),
        u32::try_from(bottom - top).expect("intersection cannot exceed either u32 height"),
    ))
}
...
impl OverlayPlacement {
    /// Fits and centers the overlay at the bottom of the primary monitor's
    /// work area. The work area, rather than raw monitor bounds, keeps it above
    /// a dock or taskbar without timing assumptions or desktop-specific gaps.
    pub fn bottom_centered(
        primary_work_area: LogicalRect,
        requested_size: LogicalSize,
        bottom_gap: u32,
    ) -> Self {
        let frame_width = requested_size.width.min(primary_work_area.width);
        let effective_gap = bottom_gap.min(primary_work_area.height);
        let usable_height = primary_work_area.height - effective_gap;
        let frame_height = requested_size.height.min(usable_height);
        let horizontal_offset = (primary_work_area.width - frame_width) / 2;
        let vertical_offset = primary_work_area.height - effective_gap - frame_height;

        Self {
            frame: LogicalRect::new(
                add_logical_offset(primary_work_area.x, horizontal_offset),
                add_logical_offset(primary_work_area.y, vertical_offset),
                frame_width,
                frame_height,
            ),
        }
...
fn add_logical_offset(origin: i32, offset: u32) -> i32 {
